...
int main(int argc, char **argv)
{
	...
	for(int i = 0; i < WS_X; i++)
	{
		...
		for(int j = 0; j < WS_Y-1; j++)
		{
			...
			for(int k = 0; k < K; k++)
			{
				omega_x[i][j][k] = IloNumVar(env,0.0,IloInfinity);
			}
		}
	}
	for(int i = 0; i < WS_X-1; i++)
	{
		...
		for(int j = 0; j < WS_Y; j++)
		{
			...
			for(int k = 0; k < K; k++)
			{
				omega_y[i][j][k] = IloNumVar(env,0.0,IloInfinity);
			}
		}
	}
	...
	while(LR_UB - LR_LB > tolerance*LR_LB  && iter < 5)
	{
		...
		for(int i = 0; i < WS_X; i++)
		{
			for(int j = 0; j < WS_Y-1; j++)
			{
				...
				for(int l = 0; l < L; l++)
				{
					...
					//cout<<"Fine"<<endl;
					//Cutting Planes
					while ((UB_best - LB) > tolerance*LB && iteration < 5)//convex constraint not satisfied, ehnce continue
					{
						cout<<"========================================================================================="<<endl;
		...
		for(int i = 0; i < WS_X; i++)
		{
			...
			for(int j = 0; j < WS_Y-1; j++)
			{
				...
				{
					cout<<"Fine!"<<endl;
					alpha_x[i][j][k] = alpha_x[i][j][k] + 2;//cplex_LR.getValue(omega_x[i][j][k]);
				}
			}
		}
		...
		for(int i = 0; i < WS_X-1; i++)
		{
			...
			for(int j = 0; j < WS_Y; j++)
			{
				...
				for(int k = 0; k < K; k++)
				{
					alpha_y[i][j][k] = alpha_y[i][j][k] + 2;//cplex_LR.getValue(omega_y[i][j][k]);
				}
			}
		}
		cout<<"-----------------------------"<<endl;
